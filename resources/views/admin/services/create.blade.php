        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('admin.services.store') }}" enctype="multipart/form-data" x-data="serviceCreateForm()">
                        @csrf
                        ...
                        <div class="mb-8 border-b pb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name')" required autofocus placeholder="e.g. Premium Link Building" x-on:input="generateSlug($event.target.value)" />
                                    ...
                                    <div class="border rounded-2xl p-6 bg-gray-50 relative overflow-hidden">
                                        ...
                                        <div class="space-y-4">
                                            ...
                                            <div>
                                                ...
                                                <textarea x-bind:id="'pkg_desc_'+pkgIndex" x-bind:name="'packages['+pkgIndex+'][description]'" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full text-sm" rows="2" x-model="pkg.description" placeholder="Short summary of what this tier includes..."></textarea>
                                            </div>
                                                ...
                                                <div class="space-y-2 mt-2">
                                                    <template x-for="(feature, fIndex) in pkg.features" :key="fIndex">
                                                        <div class="flex items-center space-x-2">
                                                            <input type="text" x-bind:name="'packages['+pkgIndex+'][features][]'" x-model="pkg.features[fIndex]" class="border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm block w-full text-xs py-1" required placeholder="e.g. 10 High DA Backlinks">
                        ...
                        <div class="mb-10 p-6 bg-indigo-50 rounded-2xl border border-indigo-100">
                            ...
                            <p class="text-sm text-gray-600 mb-6">Extra services clients can add to their order (e.g., Extra Fast Delivery, More Backlinks).</p>
                                ...
                                <template x-for="(addon, aIndex) in addons" :key="aIndex">
                                    <div class="flex flex-wrap md:flex-nowrap items-start space-x-0 md:space-x-4 space-y-4 md:space-y-0 p-4 bg-white border border-indigo-100 rounded-xl shadow-sm relative">
                                        <div class="flex-1 min-w-[200px]">
                                            ...
                                            <x-text-input x-bind:name="'addons['+aIndex+'][name]'" class="block mt-1 w-full text-sm" type="text" x-model="addon.name" required placeholder="e.g. Extra Fast Delivery" />
                                        </div>
                                        <div class="flex-1 min-w-[200px]">
                                            <x-input-label value="Description" />
                                            <x-text-input x-bind:name="'addons['+aIndex+'][description]'" class="block mt-1 w-full text-sm" type="text" x-model="addon.description" placeholder="e.g. Deliver in 24 hours" />
                                        </div>
                                        <div class="w-32">
                            ...
                            <div class="space-y-4">
                                <template x-for="(req, index) in requirements" :key="index">
                                    <div class="flex items-center space-x-4 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
                                        <div class="flex-1">
                                            <input type="text" x-bind:name="'requirements['+index+'][name]'" class="bg-gray-700 border-none text-white focus:ring-2 focus:ring-indigo-500 rounded-xl block w-full text-sm" placeholder="Field Name (e.g. Website URL)" x-model="req.name" required />
                                        </div>
                                        <div class="w-1/4">
                                        ...
                                        <div class="w-24">
                                            <label class="inline-flex items-center cursor-pointer">
                                                ...
                                                <input type="checkbox" class="rounded border-none bg-gray-600 text-indigo-500 shadow-sm focus:ring-indigo-500" x-bind:name="'requirements['+index+'][is_required]'" value="1" x-model="req.is_required" checked>
                        ...
                        <div class="flex items-center justify-between mt-8 pt-6 border-t">
                            ...
                            <div class="flex space-x-4">
                                <a href="{{ route('admin.services.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-xl text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition shadow-sm">
                                    Cancel
                                </a>
    ...
    <script>
        function serviceCreateForm() {
            return {
                ...
                requirements: [
                    { name: '', type: 'url', is_required: true }
                ],
                packages: [
                    { name: 'Basic', price: 0, emergency_fee: '', turnaround_time_days: '', express_turnaround_time_days: '', description: '', features: [''] },
                    { name: 'Standard', price: 0, emergency_fee: '', turnaround_time_days: '', express_turnaround_time_days: '', description: '', features: [''] },
                    { name: 'Premium', price: 0, emergency_fee: '', turnaround_time_days: '', express_turnaround_time_days: '', description: '', features: [''] }
                ],
                addons: [],
                faqs: [],
                addRequirement() {
                    this.requirements.push({ name: '', type: 'text', is_required: true });
                },
                ...
                addAddon() {
                    this.addons.push({ name: '', description: '', price: 0 });
                },
                removeAddon(index) {
