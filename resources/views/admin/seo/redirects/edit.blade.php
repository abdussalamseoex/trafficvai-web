<x-app-layout>
    ...
    <div class="max-w-4xl">
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('admin.seo.redirects.update', $redirect) }}" method="POST" class="p-8 space-y-6">
                @csrf @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-xs font-black text-gray-500 uppercase tracking-widest mb-2">From Path</label>
                        <input type="text" name="from_path" value="{{ old('from_path', $redirect->from_path) }}" class="w-full bg-gray-50 border-gray-200 rounded-xl p-4 font-mono text-sm" required>
                    </div>
                    <div>
                        <label class="block text-xs font-black text-gray-500 uppercase tracking-widest mb-2">To Path</label>
                        <input type="text" name="to_path" value="{{ old('to_path', $redirect->to_path) }}" class="w-full bg-gray-50 border-gray-200 rounded-xl p-4 font-mono text-sm" required>
                    </div>
                </div>
                <div>
                    ...
                    <div class="flex space-x-4">
                        <label class="flex-1 cursor-pointer">
                            <input type="radio" name="type" value="301" class="hidden peer" {{ $redirect->type == '301' ? 'checked' : '' }}>
                        ...
                        <label class="flex-1 cursor-pointer">
                            <input type="radio" name="type" value="302" class="hidden peer" {{ $redirect->type == '302' ? 'checked' : '' }}>
