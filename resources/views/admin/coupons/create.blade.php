                    ...
                    <form action="{{ route('admin.coupons.store') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="code" id="code" value="{{ old('code') }}" required class="mt-1 font-mono uppercase focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="e.g. SUMMER20">
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" min="0" name="value" id="value" value="{{ old('value') }}" required class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="20.00">
                                @error('value') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>

                            <!-- Max Uses -->
                            <div>
                                <label for="max_uses" class="block text-sm font-medium text-gray-700">Maximum Uses Limit</label>
                                <input type="number" min="1" name="max_uses" id="max_uses" value="{{ old('max_uses') }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="Leave blank for unlimited">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <div class="space-y-4 sm:flex sm:items-center sm:space-y-0 sm:space-x-10 border p-4 rounded-md bg-gray-50/50">
                                    <div class="flex items-center">
                                        <input id="target_global" name="is_global" type="radio" value="1" {{ old('is_global', '1') == '1' ? 'checked' : '' }} class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300" onclick="document.getElementById('service_selector').classList.add('hidden')">
                                        <label for="target_global" class="ml-3 block text-sm font-medium text-gray-700">
                                            Global (Any Service)
                                        </label>
                                    </div>
                                    <div class="flex items-center">
                                        <input id="target_specific" name="is_global" type="radio" value="0" {{ old('is_global') == '0' ? 'checked' : '' }} class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300" onclick="document.getElementById('service_selector').classList.remove('hidden')">
                            ...
                            <div id="service_selector" class="md:col-span-2 {{ old('is_global', '1') == '1' ? 'hidden' : '' }}">
                                ...
                                <select id="service_id" name="service_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                    ...
                                    @foreach($services as $service)
                                        <option value="{{ $service->id }}" {{ old('service_id') == $service->id ? 'selected' : '' }}>{{ $service->name }} (${{ $service->price }})</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="md:col-span-2 border-t border-gray-100 pt-6 mt-2">
                                ...
                                <div class="flex items-start">
                                    <div class="flex items-center h-5">
                                        <input id="is_private" name="is_private" type="checkbox" value="1" {{ old('is_private') ? 'checked' : '' }} class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded" onchange="document.getElementById('user_selector_wrapper').classList.toggle('hidden', !this.checked)">
                                    </div>
                            ...
                            <div id="user_selector_wrapper" class="md:col-span-2 {{ old('is_private') ? '' : 'hidden' }}">
                                ...
                                <select id="assigned_user_id" name="assigned_user_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                    ...
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}" {{ old('assigned_user_id') == $user->id ? 'selected' : '' }}>{{ $user->name }} ({{ $user->email }})</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="expires_at" id="expires_at" value="{{ old('expires_at') }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
