                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                            @foreach($methods as $slug => $gateway)
                                                ...
                                                <label class="relative cursor-pointer border bg-gray-50 hover:bg-gray-100 rounded-xl px-4 py-3 flex items-center gap-3 transition flex-1 min-w-[150px] overflow-hidden"
                                                       :class="paymentMethod === '{{ $slug }}' ? 'ring-2 ring-indigo-500 border-indigo-500 bg-indigo-50 shadow-md' : 'border-gray-200'"
                                                       @click="paymentMethod = '{{ $slug }}'">
                                                    <input type="radio" name="payment_method" value="{{ $slug }}" class="sr-only" x-model="paymentMethod">
                                                    
                                                    <!-- Badge for Automatic/Manual (Topup View) -->
                                                    ...
                                                    <div class="w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors pt-0.5"
                                                         :class="paymentMethod === '{{ $slug }}' ? 'border-indigo-600' : 'border-gray-300'">
                                                        <div class="w-2.5 h-2.5 bg-indigo-600 rounded-full" x-show="paymentMethod === '{{ $slug }}'"></div>
                                                    </div>
                                                    
                                                    <span class="text-gray-900 font-bold text-sm flex flex-col justify-center pt-1">
                                                        {{ $gateway['name'] }}
