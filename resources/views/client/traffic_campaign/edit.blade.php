<x-app-layout>
    <div class="min-h-screen bg-gray-50 dark:bg-[#0A0D14] text-gray-900 dark:text-gray-100 py-12 relative overflow-hidden transition-colors duration-300">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <form action="{{ route('client.traffic_campaign.update', $campaign) }}" method="POST" id="campaignForm">
                @csrf
                @method('PUT')
                                    ...
                                    <div class="grid grid-cols-2 gap-2 h-36 overflow-y-auto p-2 bg-gray-50 dark:bg-gray-950 border border-gray-300 dark:border-gray-800 rounded-xl">
                                        ...
                                        @foreach($allCountries as $country)
                                            <label class="flex items-center gap-2 p-1.5 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-900 cursor-pointer transition">
                                                <input type="checkbox" name="target_country[]" value="{{ $country }}" {{ in_array($country, $selectedCountries) ? 'checked' : '' }} class="rounded border-gray-300 text-orange-500 focus:ring-orange-500 bg-white dark:bg-gray-800 w-4 h-4">
