<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-4">
                <a href="{{ route('client.services.index') }}" class="inline-flex items-center text-indigo-600 hover:text-indigo-800 font-medium">
            ...
            <div class="mb-12" x-data="{ 
                ...
                getSubtotal() {
                    ...
                    if (this.selectedDelivery === 'express') {
                        ...
                        if (pkg && pkg.emergency_fee) { emergencyFee = parseFloat(pkg.emergency_fee); }
                    }
                        ...
                        <div class="mb-8">
                            <div class="flex items-baseline mb-2">
                                <span class="text-4xl font-black text-gray-900 leading-none">
                                    $<span x-text="couponApplied ? ({{ $package->price }} - getPackageDiscountAmount({{ $package->price }})).toFixed(0) : {{ $package->price }}"></span>
                                </span>
                ...
                <div class="max-w-4xl mx-auto bg-gray-900 rounded-[2.5rem] p-10 shadow-2xl overflow-hidden relative border border-gray-800">
                    ...
                    <div class="mb-8 border-t border-gray-800 pt-8" x-show="selectedPackageId">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-white font-bold">Select Payment Method</h3>
                        </div>
                        <div class="flex flex-col gap-8">
                            @foreach($gateways as $category => $methods)
                                @if(count($methods) > 0)
                                <div class="w-full">
                                    <h4 class="text-xs font-bold text-indigo-400 uppercase tracking-widest mb-4">{{ $category === 'global' ? 'Global Gateways' : ($category === 'crypto' ? 'Pay with Crypto' : ($category === 'bangladesh' ? 'Bangladesh Local (BDT)' : ucwords($category))) }}</h4>
                                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                        @foreach($methods as $slug => $gateway)
                                        <label class="cursor-pointer border border-gray-700 bg-gray-800/50 hover:bg-gray-800 rounded-xl px-4 py-4 flex items-center gap-4 transition shadow-sm" 
                                            ...
                                            <div class="flex flex-col">
                                                ...
                                                @if($slug === 'wallet')
                                                    <span class="text-xs text-indigo-400 font-bold tracking-tighter mt-1"><span class="price-convert" data-base-price="{{ auth()->user()->balance }}">${{ number_format(auth()->user()->balance, 2) }}</span> available</span>
                                                @endif
                                            </div>
                                        </label>
                                        @endforeach
                                ...
                                @endif
                            @endforeach
                        </div>      <!-- Partial Payment Option -->
                            <div class="mt-4 p-4 bg-indigo-500/10 border border-indigo-500/30 rounded-xl col-span-full" x-show="paymentMethod !== 'wallet' && walletBalance > 0">
                    ...
                    <div class="pt-8 border-t border-gray-800 flex flex-col md:flex-row items-center justify-between space-y-6 md:space-y-0 relative z-10">
                        <div>
                            ...
                            <div x-show="couponApplied && discountAmount > 0" style="display: none;" class="mb-2">
                                <span class="text-gray-400 line-through mr-2">$<span x-text="getSubtotal().toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})"></span></span>
                                <span class="text-blue-500 font-bold text-sm">Save $<span x-text="discountAmount.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})"></span></span>
                            </div>

                            <div class="flex items-baseline">
                                <span class="text-5xl font-black text-white">$<span x-text="getTotal().toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})"></span></span>
                                <span x-show="useWallet && getWalletDeduction() > 0" style="display: none;" class="ml-3 text-indigo-400 font-bold text-sm">
                                    (-$<span x-text="getWalletDeduction().toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})"></span> from wallet)
                                </span>
                            </div>
                        </div>

                        <div class="flex-1 max-w-md px-4 hidden lg:flex flex-col">
                            ...
                            <div class="flex items-center gap-2 relative">
                                <div class="relative flex-1">
                                    ...
                                    <button type="button" @click="applyCoupon()" x-show="!couponApplied" class="absolute right-1 top-1 bottom-1 bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-4 rounded-lg flex items-center transition" :class="isChecking ? 'opacity-70 cursor-not-allowed' : ''" :disabled="isChecking">
                        ...
                        <div class="w-full md:w-auto">
                            <form method="POST" :action="'/client/services/' + selectedPackageId + '/checkout'" class="flex flex-col md:flex-row md:items-end gap-4" @submit.prevent="if(!paymentMethod && getTotal() > 0) { $dispatch('notify', {type: 'error', message: 'Please select a payment method'}); return; } isProcessing = true; $el.submit();">
                                @csrf
                ...
                <div class="max-w-xl mx-auto mt-8 bg-white border border-gray-100 p-8 rounded-3xl shadow-xl flex flex-col gap-8">
                    
                    <!-- Payment Options -->
                    <div class="border-b border-gray-100 pb-6 mb-6 w-full" x-show="selectedPackageId">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-gray-900 font-bold">Select Payment Method</h3>
                        </div>
                        <div class="space-y-6">
                            @foreach($gateways as $category => $methods)
                                @if(count($methods) > 0)
                                <div class="w-full">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                        ...
                                        <label class="cursor-pointer border bg-gray-50 hover:bg-gray-100 rounded-xl px-4 py-3 flex items-center gap-3 transition flex-1 min-w-[150px]" 
                                               :class="paymentMethod === '{{ $slug }}' ? 'ring-2 ring-indigo-500 border-indigo-500 bg-indigo-50' : (('{{ $slug }}' === 'wallet' && {{ auth()->user()->balance ?? 0 }} < getTotal()) ? 'opacity-50 cursor-not-allowed border-gray-200' : 'border-gray-200')"
                                            ...
                                            <div class="w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0" :class="paymentMethod === '{{ $slug }}' ? 'border-indigo-500' : 'border-gray-300'">
                                            ...
                                            <span class="text-gray-900 font-bold text-sm flex flex-col">
                                                ...
                                                @if($slug === 'wallet')
                                                    <span class="text-[9px] font-bold uppercase tracking-wider" :class="{{ auth()->user()->balance ?? 0 }} < getTotal() ? 'text-red-500' : 'text-gray-500'">
                    ...
                    <div class="flex w-full items-end justify-between mb-4">
                        <div>
                            ...
                            <div x-show="useWallet && getWalletDeduction() > 0" style="display: none;" class="text-[10px] font-bold text-indigo-500 mt-1">
                        ...
                        <div class="flex-1 max-w-xs px-2 flex flex-col">
                            <div class="flex items-center gap-2 relative">
                                <div class="relative flex-1">
                                    <input type="text" x-model="couponCode" @keydown.enter.prevent="applyCoupon()" placeholder="Coupon?" class="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-xl py-2 pl-3 pr-10 focus:ring-indigo-500 focus:border-indigo-500 uppercase text-xs">
                                    <button type="button" @click="applyCoupon()" x-show="!couponApplied" class="absolute right-1 top-1 bottom-1 bg-brand hover:bg-orange-600 text-white font-bold px-3 rounded-lg flex items-center transition" :class="isChecking ? 'opacity-70 cursor-not-allowed' : ''" :disabled="isChecking">
                                        <span x-show="!isChecking" class="text-[10px]">Apply</span>
                                ...
                                <button type="button" x-show="couponApplied" @click="removeCoupon()" style="display:none;" class="bg-red-50 text-red-500 hover:bg-red-100 border border-red-200 p-2 rounded-xl transition" title="Remove">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                </button>
                            </div>
                            <div x-show="couponMessage" style="display:none;" class="w-full mt-1">
                                <p :class="couponError ? 'text-red-500' : 'text-blue-600 bg-blue-50 border border-blue-200 px-2 py-1 rounded'" class="text-[10px]" x-html="couponMessage"></p>
                            </div>
                        </div>
                    </div>
                    
                    <form method="POST" :action="'/client/services/' + selectedPackageId + '/checkout'" class="flex flex-wrap items-end gap-3 justify-end w-full" @submit.prevent="if(!paymentMethod && getTotal() > 0) { $dispatch('notify', {type: 'error', message: 'Please select a payment method'}); return; } isProcessing = true; $el.submit();">
                        @csrf
                        <input type="hidden" name="coupon_code" :value="couponApplied ? couponCode : ''">
                        <input type="hidden" name="payment_method" :value="paymentMethod">
                        <input type="hidden" name="use_wallet" :value="useWallet ? 1 : 0">
                        <div class="w-32 mr-2">
                            <label for="is_emergency_2" class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Delivery Option</label>
                            <select name="is_emergency" id="is_emergency_2" x-model="selectedDelivery" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                <option value="standard" x-text="'Standard' + (getPackageDeliveryDays(selectedPackageId) ? ` (${getPackageDeliveryDays(selectedPackageId)} Days)` : '')"></option>
                                <option value="express" x-show="getPackageExpressFee(selectedPackageId) > 0 || getPackageExpressDays(selectedPackageId)" x-text="'Express' + (getPackageExpressDays(selectedPackageId) ? ` (${getPackageExpressDays(selectedPackageId)} Days)` : '') + (getPackageExpressFee(selectedPackageId) > 0 ? ` (+$${getPackageExpressFee(selectedPackageId)})` : '')"></option>
                            </select>
                        </div>
                        <div class="w-48 mr-2">
                            <label for="project_id_2" class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Assign Project</label>
                            <select name="project_id" id="project_id_2" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                <option value="">None</option>
                                @if(auth()->check())
                                    @foreach(auth()->user()->projects as $project)
                                        <option value="{{ $project->id }}">{{ $project->name }}</option>
