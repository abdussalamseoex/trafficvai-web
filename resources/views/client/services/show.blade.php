                ...
                <div class="max-w-4xl mx-auto bg-gray-900 rounded-[2.5rem] p-10 shadow-2xl overflow-hidden relative border border-gray-800">
                                                ...
                                                <div class="flex items-center gap-3">
                                                    @if(isset($gateway['logo']))
                                                        <div class="h-10 w-12 flex items-center justify-center">
                                                            <img src="{{ $gateway['logo'] }}" alt="{{ $gateway['name'] }}" class="max-h-full max-w-full object-contain opacity-90 group-hover:opacity-100 transition-opacity">
                                                        </div>
                                                    @endif
                    ...
                    <div class="pt-8 border-t border-gray-800 relative z-10">
                        ...
                        <div class="bg-gray-800/40 rounded-[2rem] p-8 border border-gray-700/50">
                            <form method="POST" :action="'/client/services/' + selectedPackageId + '/checkout'" class="flex flex-col lg:flex-row items-center lg:items-end gap-8" @submit.prevent="if(!paymentMethod && getTotal() > 0) { $dispatch('notify', {type: 'error', message: 'Please select a payment method'}); return; } isProcessing = true; $el.submit();">
                                @csrf
                ...
                <div class="max-w-xl mx-auto mt-8 bg-white border border-gray-100 p-8 rounded-3xl shadow-xl flex flex-col gap-8">
                    ...
                    <div class="pt-8 border-t border-gray-100 relative z-10 w-full">
                        ...
                        <div class="bg-gray-50 rounded-3xl p-6 border border-gray-100">
                            <form method="POST" :action="'/client/services/' + selectedPackageId + '/checkout'" class="flex flex-col lg:flex-row items-center lg:items-end gap-5" @submit.prevent="if(!paymentMethod && getTotal() > 0) { $dispatch('notify', {type: 'error', message: 'Please select a payment method'}); return; } isProcessing = true; $el.submit();">
                                @csrf
                                <input type="hidden" name="coupon_code" :value="couponApplied ? couponCode : ''">
                                <input type="hidden" name="payment_method" :value="paymentMethod">
                                <input type="hidden" name="use_wallet" :value="useWallet ? 1 : 0">
