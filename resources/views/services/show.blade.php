                    ...
                    <div class="mb-8 border-t border-gray-800 pt-8" x-show="selectedPackageId">
                        ...
                        <div class="flex flex-col gap-6">
                            @foreach($gateways as $category => $methods)
                                @if(count($methods) > 0)
                                <div>
                                    <h4 class="text-xs font-bold text-indigo-500 uppercase tracking-widest mb-3">{{ $category === 'global' ? 'Global Gateways' : ($category === 'crypto' ? 'Pay with Crypto' : ($category === 'bangladesh' ? 'Bangladesh Local (BDT)' : ucwords($category))) }}</h4>
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div class="flex flex-col">
                                                    ...
                                                    @if($slug === 'wallet' && auth()->check())
                                                        <span class="text-[10px] text-indigo-400 font-bold tracking-tighter">${{ number_format(auth()->user()->balance, 2) }} available</span>
                                                    @endif
                                                </div>
                                            </div>
                                        </label>
                        ...
                        <div class="flex flex-col sm:flex-row gap-4">
                            ...
                            @foreach($gateways as $category => $methods)
                                ...
                                <label class="cursor-pointer border bg-gray-50 hover:bg-gray-100 rounded-xl px-4 py-3 flex items-center gap-3 transition flex-1 min-w-[150px]" 
                                       :class="paymentMethod === '{{ $slug }}' ? 'ring-2 ring-indigo-500 border-indigo-500 bg-indigo-50' : (('{{ $slug }}' === 'wallet' && @auth {{ auth()->user()->balance }} < getTotal() @else true @endauth) ? 'opacity-50 cursor-not-allowed border-gray-200' : 'border-gray-200')"
