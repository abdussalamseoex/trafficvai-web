...
class WebsiteTrafficController extends Controller
{
    public function index()
    {
        ...
        return view('admin.traffic.index', compact('services'));
    }

    public function create()
    {
        $categories = \App\Models\Category::where('is_active', true)->where('type', 'service')->get();
        return view('admin.traffic.create', compact('categories'));
    }
    ...
    public function edit(\App\Models\Service $traffic)
    {
        $traffic->load('requirements');
        $categories = \App\Models\Category::where('is_active', true)->where('type', 'service')->get();
        // Passing it as $service to match the copied blade view
        $service = $traffic;
        return view('admin.traffic.edit', compact('service', 'categories'));
    }

    public function update(Request $request, \App\Models\Service $traffic)
    {
        $service = $traffic;
        $validated = $request->validate([
            'name' => 'required|string|max:255',
    ...
    public function destroy(\App\Models\Service $traffic)
    {
        $traffic->delete();
        return redirect()->route('admin.traffic.index')->with('success', 'Traffic package deleted successfully.');
    }
}
